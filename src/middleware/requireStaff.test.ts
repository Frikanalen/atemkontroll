...
import { V2AuthenticatedSession, getProfile } from "../fkapi/getProfile.js";
...
describe("requireStaff", () => {
  ...
  it("throws a 403 if the user is not staff", async () => {
    const mockData: V2AuthenticatedSession = {
      authenticated: true,
      user: { permissions: [], email: "test@test.com" },
    };

    (getProfile as jest.Mock).mockResolvedValue(mockData);

    const response = await request(app.callback()).get("/").set("Cookie", ["fk-session=testSessionId"]);
  ...
  it("continues to the next middleware if the user is staff", async () => {
    const mockData: V2AuthenticatedSession = {
      authenticated: true,
      user: { permissions: ["ATEM_CONTROL"], email: "test@test.com" },
    };
